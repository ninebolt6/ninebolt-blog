  ...
  <div>
    ...
    <div class="index flex xl:px-32 lg:px-8 md:px-4 flex-col lg:flex-row bg-gray-200">
      <div class="w-auto lg:w-3/4 mx-4">
        <div class="mt-4">
          <p>タグ<span class="py-2 text-red-600">{{ $route.params.slug }}</span>がついた記事: {{ articles.length }}件</p>
        </div>
...
export default {
  head() {
    ...
    if (typeof this.$route.params.slug !== "undefined") {
      tagStr = this.$route.params.slug
      titleStr = tagStr + " | タグ検索"
    } else {
      tagStr = ""
      titleStr = "タグ検索"
    }
    return {
      title: titleStr,
      meta: [
        { hid: "description", name: "description", content: "タグ検索結果: " + tagStr },
        { hid: "og:site_name", property: "og:site_name", content: "NineBolt" },
        { hid: "og:type", property: "og:type", content: "website" },
        {
          hid: "og:url",
          property: "og:url",
          content: "https://ninebolt.net/tag/" + tagStr + "/"
        },
        {
          hid: "og:title",
          property: "og:title",
